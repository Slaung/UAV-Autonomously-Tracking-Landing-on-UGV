...
- FNN_height_predicter 為FNN高度預測程式
- Tracking_Land_Control.py 為無人機的主控程式
- marker_detection.py 為YOLO-tiny、ArUco marker之影像檢測
- pid_controller_yolo.py 為追蹤PD控制器
- pid_controller_aruco_marker.py 為降落PD控制器
- fuzzy_controller_x_gain.py 為X軸模糊控制
- fuzzy_controller_y_gain.py 為Y軸模糊控制
...
- 降落成功率: 95% (成功降落在車上的次數:19 / 總實驗次數:20)
- 成功降落之花費平均時間: 18.55s (成功降落在車上之總花費時間:371 / 成功降落之總實驗次數:20)
- 成功追回率: 83.33% (爬升後順利追回至UGV上方的總次數: 10 / 爬升總次數: 12)
